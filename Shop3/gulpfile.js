...
gulp.task("clean", ["clean:adminJs", "clean:clientJs", "clean:adminCss", "clean:clientCss"]);  // gọi các task trên

// minify
gulp.task("min:adminJs", function () {
    return gulp.src([paths.adminJs, "!" + paths.minJs], { base: "." }) // min js trừ các file đã min
        .pipe(concat(paths.concatJsAdminDest))
        .pipe(uglify()) // làm xấu
        .pipe(gulp.dest(".")); // đẩy ra
});
...
gulp.task("min", ["min:adminJs", "min:clientJs", "min:adminCss", "min:clientCss"]); // gọi các task trên